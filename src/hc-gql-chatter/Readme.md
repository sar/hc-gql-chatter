...
WORKDIR /src
COPY ["hc-gql-chatter.csproj", "hc-gql-chatter/"]
RUN dotnet restore "hc-gql-chatter/hc-gql-chatter.csproj"

WORKDIR "/src/hc-gql-chatter"
COPY . .
RUN dotnet build "hc-gql-chatter.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "hc-gql-chatter.csproj" \
...
Execute unit tests
```
    cd "hc-gql-chatter/test/hc-gql-chatter.Tests"
    dotnet test
```

Deploy application
```
    cd "hc-gql-chatter/src/hc-gql-chatter"
